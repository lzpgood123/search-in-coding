...
...
fields let you restrict this:
...
: fork`, you
...
in your skill and pick
...
to execute it. The built-
...
](/en/sub-
...
...
...
deferred and discovered on demand. Claude Code
...
it by default on Google Cloud's Agent Platform. It is also disabled when `ANTHROPIC_BASE_URL` points to a non-first-party host, since most proxies don't forward `tool_reference` blocks. Set `ENABLE_TOOL_SEARCH` explicitly to override either fallback
...
...
cp add` command
...
of three scopes, stored across two files, depending on the `--
...
` flag. You don't need to edit these
...
knowing where they are helps with
...
and version control.
...
...
Highlights:
Comprehensive Claude Code ecosystem plugin with official documentation management, meta-skills for all Claude Code features, development
...
, auditing agents, and observability hooks.
...
...
-delegation`
...
Audit docs-management delegation compliance
...
| `/claude-ecosystem
...
` | Audit
...
...
search
- Meta-skills: hooks, memory, skills, MCP,
...
, plugins, subagents with noun-phrase naming
...
...
...
overlap, framework match (from
...
`package.
...
detection), and trust tier. Same query always returns the
 same ranking.
...
- **Cross-vendor by design.**
...
.md is a multi-vendor format
...
