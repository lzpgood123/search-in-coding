...
...
. You can type `/skill-name` to invoke it directly, and Claude can load it automatically when relevant to your conversation. Two front
...
...
...
to execute it. The built-in Explore
...
agents [skip
...
/sub-
...
...
Code to your tools with the Model Context Protocol.
...
...
...
starts as a subprocess on your
...
service it reaches over
...
URL. Use one for tools that need access to local resources like
...
browser, your filesystem, or a database socket
...
: it gives
...
...
0017
...
Claude Code Skills Over MCP for Ecosystem Knowledge
...
...
Title: mcfearsome/cc-mcp-executor-skill
URL: https://github.com/mcfearsome/cc-mcp-executor-skill
Published: 2025-11-11T17:26:59.000Z
Author: N/A
Highlights:
...
...
you symlink `~/.claude/subagent-mcp.json` to `~/.mcp.json`, or globally export `MCP_CONFIG_PATH`, then **main Claude Code will load all MCP servers**, defeating the entire purpose (98% token reduction).
...
The subagent sets `MCP_CONFIG_PATH` temporarily in the Bash execution command:
...
Claude Code has NO MCP servers loaded (keeps context clean)
...
- ✅ Subagent
...
sets `MCP_CONFIG_PATH` temporarily
...
- ✅ MCP client reads from subagent config only during code
...
...
## How It Works
...
cached script pattern
...
adapted TypeScript/Python code
...
...
...
Complete redesign from code-
...
no longer needs MCP servers configured
...
Subagents handle
...
with local MCP
