...
const SIC_charts = {
  _niceMax(maxVal) {
    ...
    if (m <= 5) return m;
    // Prefer tighter ceiling so tall bars fill ~80–100% of plot (avoid 2625 → 5000).
  ...
  _verticalBarChart(data, maxVal, options) {
    ...
    const padT = 22;
    const padB = 36;
    // Default taller plot so score card matches tool coverage visual weight
    const chartH = Math.max(160, Number(options.height) || 280);
    ...
    const n = data.length;
    const plotW = Math.max(n * (minBarW + barGap) + barGap, 280);
    ...
    const bars = data.map((d, i) => {
      const val = Math.max(0, Number(d.value) || 0);
      const h = Math.max(val > 0 ? 3 : 0, (val / niceMax) * chartH);
      const x = padL + barGap + i * (barW + barGap);
      const y = padT + chartH - h;
      const label = String(d.label || '').slice(0, labelMax);
      const valueY = Math.max(padT + 12, y - 5);
      return '<rect x="' + x + '" y="' + y + '" width="' + barW + '" height="' + h + '" fill="var(--color-accent)" rx="5"/>' +
        '<text x="' + (x + barW / 2) + '" y="' + valueY + '" text-anchor="middle" font-size="11" font-weight="600" fill="var(--color-text-secondary)">' + val + '</text>' +
        '<text x="' + (x + barW / 2) + '" y="' + (padT + chartH + 16) + '" text-anchor="middle" font-size="11" fill="var(--color-text-muted)">' + this._escText(label) + '</text>';
    }).join('');
    ...
    // Fill card width; no aggressive max-height so plot reads as tall as the left chart
    return '<svg viewBox="0 0 ' + width + ' ' + height + '" style="width:100%;height:auto;min-height:' + height + 'px;" role="img" aria-label="' + this._escAttr(options.ariaLabel || 'chart') + '">' + grid + axis + bars + '</svg>';
  },
